...
describe('Task runner test', function () {
    ...
    it('should be success no cancel scan', function (done) {
        ...
        console.log(tr.stdout);
        assert.strictEqual(tr.stdout.indexOf('Pipeline not cancelled, nothing to do.') >= 0, true, "should display cleanup message: Pipeline not cancelled, nothing to do.");
        done();
    });
    ...
    it('should be success cancel scan', function (done) {
        ...
        tr.run(10);
        console.log(tr.stdout);
        assert.strictEqual(tr.stdout.indexOf('Canceling scan with ID') >= 0, true, "should display cleanup message: Canceling scan with ID");
        done();
    });

    it('should be success cancel before scan start', function (done) {
        this.timeout(300000);
        const tp = path.join(__dirname, 'success_cancel_before_scan.js');
        const tr: ttm.MockTestRunner = new ttm.MockTestRunner(tp);
        tr.run(10);
        console.log(tr.stdout);
